...
const getAll = async ctx => {
  const { status, error, skip } = await validateSkipValue(
    parseInt(ctx.query.pageId || 1)
  );
...
const addToPendingPlace = async ctx => {
  ...
  const { status, error, data } = await validatePendingPlaceData({
    seanceId,
    x,
    y
  });
...
const removeFromPending = async ctx => {
  ...
  try {
    currentSeance.pendingPlaces = currentSeance.pendingPlaces.filter(
...
const search = async ctx => {
  ...
  const { status, error, data } = await validateSearchData({
    ...ctx.request.body,
    skip
  });
  ...
  if (filter === "city") {
    result = _result.filter(item =>
      item.cinema.city.toLowerCase().includes(text.toLowerCase())
    );
    count = result.length;
  } else if (filter === "cinema") {
    result = _result.filter(item =>
      item.cinema.name.toLowerCase().includes(text.toLowerCase())
    );
    count = result.length;
  } else if (filter === "film_name") {
    result = _result.filter(item =>
      item.film.name.toLowerCase().includes(text.toLowerCase())
    );
    count = result.length;
  } else if (filter === "date") {
  ...
  ctx.body = 200;
  ctx.body = {
    data: util.optimizeSessions(
      result.slice((skip - 1) * 9, (skip - 1) * 9 + 9)
    ),
    count
  };
};
