...
const isMoreThanOnePlaceExist = sessions => {
  ...
  return sessions.filter(item => {
    if (!item.selectedPlaces && !item.pendingPlaces) return true;
    ...
    if (
      item.selectedPlaces.length === placeCount ||
      item.pendingPlaces.length === placeCount
    )
      return false;
...
const addRoomTypes = (_types, _session) => {
  const types = deepCopy(_types).map(el => {
    delete el._id;
    return el;
  });

  let result = findPlaceTypes(
    _session.cinema.rooms[_session.roomNumber].placeSchema
  );
