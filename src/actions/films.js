...
export const onFilmsLoad = pageId => {
  return (dispatch, getState) => {
    ...
    if (!pageId) pageId = getState().films.pageId + 1;

    return axios
      .get(api.sessions + pageId)
...
export const onFilmLoad = seanceId => {
  return (dispatch, getState) => {
    ...
    if (!seanceId) seanceId = getState().films.openedSeance._id;

    return axios
      .get(`${api.sessionsById}/${seanceId}`)
