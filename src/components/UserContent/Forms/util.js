export const validate = (values, props) => {
  ...
  if ("name" in values) {
    if (values.name.length < 3) {
      ...
      errors.name =
        "The name must have only one letter in upper case, the one with which it begins.!";
    }
  }
  ...
  if (!values.password) {
    ...
    errors.password =
      "The password is too weak! You can use characters in lowercase and upper case.";
  }
...
export const handleLogin = (values, actions) => {
  return actions.props.userActions.login({
    email: values.email,
...
export const handleRegister = (values, actions) => {
  return actions.props.userActions.register({
    name: values.name,
    email: values.email,
    password: values.password
  });
