...
class ChangeMatrix extends PureComponent {
  validateChangedValue = (x, y, value) => {
    ...
    if (value > maxValue || value < 0) return false;
  ...
  onChangeMatrixValue = (x, y, event) => {
    ...
    if (!this.validateChangedValue(x, y, value)) return;
    ...
    for (let i = 0; i <= maxValue; i++) {
      if (value === i) {
        ...
        // not update value
        if (i > 1 && matrix[x].length < y + i) return;
        ...
        if (matrix[x].length < y + i) return;

        for (let j = 1; j < i; j++) updateMatrix(x, y + j, value);
      }
    }
  };
            ...
            <div key={rowIndex} className="matrix-row">
              {row.map((item, columnIndex) => (
                <div key={columnIndex} className="matrix-row__item">
                  <input
                    onChange={event =>
                      this.onChangeMatrixValue(rowIndex, columnIndex, event)
                    }
                    type="number"
                    value={item}
                  />
