...
class InnerSeance extends PureComponent {
  ...
  handleDateChangeWrapper = event => {
    ...
    if (!value || value > max) target.value = 0;
    ...
    return (
      <form onSubmit={handleSubmit} className="default-form" noValidate>
        ...
        <input
          ...
          min={0}
          name="roomNumber"
          onChange={this.handleDateChangeWrapper}
          value={values.roomNumber}
          max={
            cinema.selectedCinema ? cinema.selectedCinema.rooms.length - 1 : 9
          }
        />

        <button type="submit" className="btn" disabled={seance.isFetching}>
          Add
        </button>
      </form>
    );
