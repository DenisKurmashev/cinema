...
class FilmList extends PureComponent {
  ...
  handleFilmsItemClick = event => {
    ...
    if (
      ...
      !target.parentNode.hasAttribute(attrName)
    )
      return;
    ...
    if (films.selectedFilm) if (selectedId === films.selectedFilm._id) return;
  ...
  getSelectedFilms = () => {
    ...
    return (
      <div
        ...
        className="cinema-content-selected-item"
      >
  ...
  render() {
    ...
    return (
      <div className="cinema-content">
        ...
        {!this.state.isListVisible ? null : films.isFetching ? (
          <FilmLoading />
        ) : (
          <Fragment>
            <div
              className="cinema-content-list"
              onClick={this.handleFilmsItemClick}
            >
              {films.loadedFilms.map((el, index) => (
                <div
                  data-index={el._id}
                  key={index}
                  className={"list-item" + (index % 2 === 0 ? " gray" : "")}
                >
                  <div>{el.name}</div>
                </div>
              ))}
