...
const validatePendingPlaceData = async (data) => {
    const validateSchema = joi.object().keys({
        seanceId: joi.string().min(24).required(),
...
const validateMongoId = async (id) => {
    ...
    try {
        await joi.validate(id, joi.string().min(24).required());
    } catch(ex) {
        result.status = false;
        result.error  = ex; 
    }
...
const validateNewSeanceData = async (data) => {
    ...
    const validateSchema = joi.object().keys({
        cinema:     joi.string().min(24).required(),
        film:       joi.string().min(24).required(),
...
const validateSearchData = async (data) => {
    ...
    const validateSchema = joi.object().keys({
        filter: joi.string().min(4).valid("city", "cinema", "film_name", "date", "one_place_exist").required(),
        text:   data.filter !== "date" ? joi.string().required() : joi.date().required(),
